                    ...
                    if dosya_yolu.endswith('.udf'):
                        ...
                        if icerik:
                            story.extend(self._olustur_evrak_sayfasi(
                                os.path.basename(dosya_yolu), icerik, styles
                            ))
                            metin_var = True
                except Exception as e:
                    rapor.hatalar.append(f"UDF okuma hatası: {e}")
    ...
    def _olustur_stiller(self) -> dict:
        ...
        return {
            'baslik': ParagraphStyle('Baslik', parent=styles['Heading1'],
                fontName=self.font_name, fontSize=18,
            ...
            'alt_baslik': ParagraphStyle('AltBaslik', parent=styles['Heading2'],
                ...
                textColor=colors.HexColor('#2C5282'), alignment=TA_CENTER, spaceAfter=10),
            'normal': ParagraphStyle('Normal', parent=styles['Normal'],
                fontName=self.font_name, fontSize=10, leading=14,
                alignment=TA_JUSTIFY, spaceAfter=6),
            'evrak_baslik': ParagraphStyle('EvrakBaslik', parent=styles['Heading3'],
                fontName=self.font_name, fontSize=11, textColor=colors.white,
                backColor=colors.HexColor('#2C5282'), alignment=TA_LEFT,
                spaceBefore=10, spaceAfter=10, leftIndent=5, rightIndent=5),
            'footer': ParagraphStyle('Footer', parent=styles['Normal'],
                fontName=self.font_name, fontSize=8, textColor=colors.grey, alignment=TA_CENTER)
        }
    ...
    def _oku_udf(self, udf_yolu: str) -> str:
        try:
            with zipfile.ZipFile(udf_yolu, 'r') as zf:
                ...
                text = match.group(1) if match else re.sub(r'<[^>]+>', ' ', raw)
