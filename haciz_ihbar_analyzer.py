...
# -*- coding: utf-8 -*-
"""
HACİZ İHBAR ANALYZER v5.0 (Oracle Enhanced)
=========================================
Banka cevaplarını analiz eder. App.py ile tam uyumlu Enum yapısı kullanır.
Oracle-Grade "Negative-First" algoritması ile donatılmıştır.
"""
...
class HacizIhbarAnalyzer:
    ...
    def __init__(self):
        ...
        self.BLOKE_CONTEXT = re.compile(
            r'(?:bloke|haciz|tedbir|mahsus)(?:.{0,60}?)(?:tutar|bedel|miktar|bakiye)?.{0,20}?'
            r'(\d{1,3}(?:[.,]\d{3})*(?:[.,]\d{2})?)', 
    ...
    def batch_analiz(self, dosya_yollari: List[str]) -> HacizIhbarAnalizSonucu:
        ...
        # We assume app.py handles initial extraction but we handle deep traversal
        all_files = []
        ...
        for fp in all_files:
            try:
                # Use Core utility for UDF/PDF/TXT reading
    ...
    def analyze_response(self, text: str, filename: str) -> HacizIhbarCevabi:
        ...
        # 3. Positive Match
            ...
            if match:
                ...
                if tutar > 5.0: # Filter noise
                    durum = CevapDurumu.BLOKE_VAR
                    sonraki = "MAHSUP TALEBİ GÖNDER!"
                else:
                    durum = CevapDurumu.HESAP_VAR_BAKIYE_YOK
                    sonraki = "89/2 Gönder"
            elif any(x in text_clean for x in ["bloke konulmuştur", "haciz uygulanmıştır", "şerh işlenmiştir"]):
                durum = CevapDurumu.BLOKE_VAR
        ...
        return HacizIhbarCevabi(
            muhatap=muhatap_adi if muhatap_adi != "Bilinmeyen" else filename,
